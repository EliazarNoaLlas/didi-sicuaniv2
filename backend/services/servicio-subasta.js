...
class ServicioSubasta {
  ...
  async asignarViajeAConductor(idSolicitudViaje, idConductor, idOferta) {
    ...
    // Calcular precio acordado: usar precio_ofrecido si existe, sino precio_ofrecido_pasajero, sino precio_sugerido
    let precioAcordado = oferta.precio_ofrecido;
    if (!precioAcordado || precioAcordado === null) {
      precioAcordado = solicitudViaje.precio_ofrecido_pasajero || solicitudViaje.precio_sugerido_soles;
    }

    // 2. Actualizar solicitud de viaje con conductor asignado
    await SolicitudViaje.findByIdAndUpdate(idSolicitudViaje, {
      estado: 'asignado',
      id_conductor_asignado: idConductor,
      fecha_asignacion: ahora,
      precio_final_acordado: precioAcordado,
    });
    ...
    // 4. Rechazar todas las otras ofertas
    ...
    // 5. Marcar conductor como no disponible
    await Usuario.findByIdAndUpdate(idConductor, {
      'informacion_conductor.esta_disponible': false,
    });

    // 6. Obtener información completa para notificaciones
    const conductor = await Usuario.findById(idConductor);
    ...
    // Obtener información del conductor (puede estar en informacion_conductor o en el objeto principal)
    const infoConductor = conductor.informacion_conductor || {};
    const distanciaConductor = oferta.distancia_conductor_km || solicitudViaje.distancia_estimada_km || 0;
    const tiempoLlegada = oferta.tiempo_estimado_llegada_min || solicitudViaje.duracion_estimada_min || 0;

    // 7. Notificar vía Socket.io
    if (io) {
      ...
      // Notificar al PASAJERO con todos los datos necesarios
      io.to(`usuario:${solicitudViaje.id_pasajero}`).emit('viaje:aceptado', {
        idSolicitudViaje: idSolicitudViaje.toString(),
        idOferta: idOferta.toString(),
        idConductor: idConductor.toString(),
        // Datos del conductor en español (para compatibilidad)
        nombreConductor: conductor.nombre || 'Conductor',
        ...
        tipoVehiculo: infoConductor.tipo_vehiculo || 'taxi',
        placaVehiculo: infoConductor.placa_vehiculo || '',
        modeloVehiculo: infoConductor.modelo_vehiculo || '',
        colorVehiculo: infoConductor.color_vehiculo || '',
        // Precio acordado
        precioAcordado: precioAcordado,
        // Direcciones
        direccionOrigen: solicitudViaje.origen_direccion,
        direccionDestino: solicitudViaje.destino_direccion,
        // Distancia y tiempo (usando nombres que el frontend espera)
        distancia: distanciaConductor,
        duracion: tiempoLlegada,
        driverDistanceKm: distanciaConductor,
        driverEtaMin: tiempoLlegada,
        // Datos adicionales en inglés para compatibilidad
        driverName: conductor.nombre || 'Conductor',
        driverEmail: conductor.correo || '',
        driverPhone: conductor.telefono || '',
        driverRating: infoConductor.calificacion || 5.0,
        driverTotalRides: infoConductor.total_viajes || 0,
        vehicleType: infoConductor.tipo_vehiculo || 'taxi',
        vehiclePlate: infoConductor.placa_vehiculo || '',
        vehicleModel: infoConductor.modelo_vehiculo || '',
        vehicleColor: infoConductor.color_vehiculo || '',
        agreedPrice: precioAcordado,
        originAddress: solicitudViaje.origen_direccion,
        destinationAddress: solicitudViaje.destino_direccion,
        // Mensaje y estado
